...
if __name__=='__main__':
	print ('corr'+'\t'+str(round(cal_pearson(x,y)/sqrt(float(h2)),3)))
	g.write('corr'+'\t'+str(round(cal_pearson(x,y)/sqrt(float(h2)),3))+'\n')
########################################################  REG(not use)
#import numpy as np
#import pandas as pd
##x = np.array([2,3,4,6])
...
g.write('MSE'+'\t'+str(round(mse,3))+'\n')
g.write('MAE'+'\t'+str(round(mae,3))+'\n')


#---h2
# log=open('remlf90.log')
# H2=open(result+'_h2','a')
# while True:
	# line=log.readline()
	# if line=='':
		# break
	# if line.startswith('Genetic'):
		# va=log.readline()
	# if line.startswith('Residual'):
		# ve=log.readline()	
# h2=float(va)/(float(va)+float(ve))
# H2.write(str(h2)+'\n')
# H2.close()
